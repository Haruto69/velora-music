...
export function QueueItem({ song, index, isActive, onRemove, onPlay }: QueueItemProps) {
  ...
  return (
    <div className={`flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-white/10 group transition-colors ${isActive ? 'bg-white/5' : ''}`}>
      <div 
        onClick={() => onPlay(index)}
        className="relative w-10 h-10 rounded overflow-hidden flex-shrink-0 bg-white/10 shadow-sm cursor-pointer"
      >
