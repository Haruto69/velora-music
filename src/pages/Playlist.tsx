...
export default function Playlist() {
  ...
  const totalDuration = calculateDuration(playlistSongs);
      ...
      <div className="relative w-full p-6 md:p-10 pt-12 md:pt-16">
        ...
        <div className="relative z-10 w-full">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6 w-full">
            <div className="w-full max-w-[12rem] md:max-w-[15rem] aspect-square shadow-2xl flex-shrink-0 group rounded-lg overflow-hidden">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left mt-4 md:mt-0">
            <span className="uppercase text-xs font-bold tracking-wider mb-2">Playlist</span>
            <h1 className="text-4xl md:text-6xl md:leading-tight font-bold mb-4 text-white line-clamp-2">{playlist.title}</h1>
            <p className="text-white/70 mb-4 max-w-xl line-clamp-2">{playlist.description}</p>
            <div className="flex items-center gap-2 text-sm text-white/80 font-medium">
              <span className="text-white hover:underline cursor-pointer">{playlist.createdBy}</span>
              <span>•</span>
              <span>{playlistSongs.length} songs</span>
              <span>•</span>
              <span className="opacity-75">{Math.floor(totalDuration / 60)} min {totalDuration % 60} sec</span>
            </div>
          </div>
        </div>
        </div>
